...
import { rankCreateController } from 'src/features/rank/controllers/rankCreateController';
import { rankDestroyManyController } from 'src/features/rank/controllers/rankDestroyManyController';
import { rankFindManyController } from 'src/features/rank/controllers/rankFindManyController';
...
export async function POST(request: NextRequest) {
  ...
  try {
    ...
    const payload = await rankCreateController(body, context);
...
export async function GET(request: NextRequest) {
  ...
  try {
    ...
    const payload = await rankFindManyController(query, context);
    return NextResponseSuccess(request, context, payload);
  } catch (error: any) {
    return NextResponseError(request, context, error);
  }
...
export async function DELETE(request: NextRequest) {
  ...
  try {
    context = await appContext(request);
    const query = queryToObject(request.nextUrl.search);
    await rankDestroyManyController(query, context);
    const payload = true;
    return NextResponseSuccess(request, context, payload);
  } catch (error: any) {
    return NextResponseError(request, context, error);
