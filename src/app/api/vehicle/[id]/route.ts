...
import { vehicleFindController } from 'src/features/vehicle/controllers/vehicleFindController';
...
export async function GET(
  ...
  try {
    ...
    const payload = await vehicleFindController(params, context);
...
export async function PUT(
  ...
  try {
    ...
    const payload = await vehicleUpdateController(params, body, context);
    return NextResponseSuccess(request, context, payload);
  } catch (error: any) {
    return NextResponseError(request, context, error);
  }
