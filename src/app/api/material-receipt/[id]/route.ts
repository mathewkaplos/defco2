...
import { materialReceiptUpdateController } from 'src/features/materialReceipt/controllers/materialReceiptUpdateController';
...
export async function GET(
  ...
  try {
    ...
    const payload = await materialReceiptFindController(params, context);
...
export async function PUT(
  ...
  try {
    ...
    const payload = await materialReceiptUpdateController(params, body, context);
    return NextResponseSuccess(request, context, payload);
  } catch (error: any) {
    return NextResponseError(request, context, error);
  }
