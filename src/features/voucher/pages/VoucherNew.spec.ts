...
async function goToVoucherNew(page: Page) {
  ...
  await page.goto(`/voucher/new`);
  await page.waitForURL(`/voucher/new`);
}

test.beforeEach(async ({ page }) => {
  await testCleanupDatabase();
  await setupAdminForTest();
  await goToVoucherNew(page);
});

test('renders page', async ({ page }) => {
  await expect(
    page.getByRole('heading', { name: dictionary.voucher.new.title }),
  ).toBeTruthy();
});
