...
async function goToVoucherList(page: Page) {
  ...
  await page.waitForURL(`/voucher`);
}

test.beforeEach(async ({ page }) => {
  await testCleanupDatabase();
  await setupAdminForTest();
  await goToVoucherList(page);
});

test('renders page', async ({ page }) => {
  await expect(
    page.getByRole('heading', { name: dictionary.voucher.list.title }),
  ).toBeTruthy();
});
