...
async function goToVoucherForm(page: Page) {
  ...
  await page.goto(`/voucher/${voucher.id}/edit`);
  await page.waitForURL(`/voucher/${voucher.id}/edit`);
}

test.beforeEach(async ({ page }) => {
  await testCleanupDatabase();
  await setupAdminForTest();
  await goToVoucherForm(page);
});
