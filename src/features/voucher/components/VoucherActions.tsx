...
import { voucherDestroyManyApiCall } from 'src/features/voucher/voucherApiCalls';
...
export function VoucherActions({
  ...
  const hasPermissionToEdit = hasPermission(permissions.voucherUpdate, context);
  ...
  const destroyMutation = useMutation({
    mutationFn: () => {
      return voucherDestroyManyApiCall([voucher.id]);
    },
    onSuccess: () => {
      ...
      toast({
        description: dictionary.voucher.destroy.success,
      });
    },
    onError: (error: any) => {
      toast({
  ...
  return (
    <div className="flex justify-end gap-2">
        ...
        <DropdownMenuContent align="end" className="w-[160px]">
          {mode === 'table' && (
            <DropdownMenuItem asChild>
              <Link href={`/voucher/${voucher.id}`} prefetch={false}>
          ...
          {mode === 'table' && hasPermissionToEdit && (
            <DropdownMenuItem asChild>
              <Link href={`/voucher/${voucher.id}/edit`} prefetch={false}>
      ...
      {destroyDialogOpen && (
        <ConfirmDialog
          title={dictionary.voucher.destroy.confirmTitle}
