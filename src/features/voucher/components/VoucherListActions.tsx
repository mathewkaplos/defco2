...
import {
  voucherDestroyManyApiCall,
  voucherFindManyApiCall,
} from 'src/features/voucher/voucherApiCalls';
import { voucherExporterMapper } from 'src/features/voucher/voucherExporterMapper';
import { voucherFilterInputSchema } from 'src/features/voucher/voucherSchemas';
...
export default function VoucherListActions({
  context,
  table,
  count,
  filter,
  ...
  filter: z.input<typeof voucherFilterInputSchema>;
  ...
  const hasPermissionToCreate = hasPermission(permissions.voucherCreate, context);
  ...
  const hasPermissionToImport = hasPermission(permissions.voucherImport, context);
  ...
  const exportMutation = useMutation({
    ...
    onSuccess: (data) => {
      csvExporter(
        voucherExporterMapper(data.vouchers, context),
        dictionary.voucher.fields,
        'vouchers',
      );
      toast({
        description: dictionary.voucher.export.success,
      });
    },
    onError: (error: any) => {
      toast({
  ...
  const destroyMutation = useMutation({
    mutationFn: () => {
      ...
      if (!ids.length) {
        throw new Error(context.dictionary.voucher.destroyMany.noSelection);
      }

      return voucherDestroyManyApiCall(ids);
    },
    onSuccess: () => {
      ...
      toast({
        description: dictionary.voucher.destroyMany.success,
      });
    },
    onError: (error: any) => {
      toast({
  ...
  return (
    <>
        ...
        <DropdownMenuContent>
          ...
          {hasPermissionToImport && (
            <DropdownMenuItem asChild>
              <Link href={`/voucher/importer`} prefetch={false}>
      ...
      {destroyManyDialogOpen && (
        <ConfirmDialog
          title={dictionary.voucher.destroyMany.confirmTitle}
          description={formatTranslation(
            dictionary.voucher.destroyMany.confirmDescription,
            selectedCount,
          )}
