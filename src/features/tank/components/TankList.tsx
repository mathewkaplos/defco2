...
import TankListActions from 'src/features/tank/components/TankListActions';
import TankListFilter from 'src/features/tank/components/TankListFilter';
import { tankFindManyApiCall } from 'src/features/tank/tankApiCalls';
import {
  TankWithRelationships,
  tankFilterInputSchema,
...
import { TankNewButton } from 'src/features/tank/components/TankNewButton';
import { z } from 'zod';
import { Tank } from '@prisma/client';

const defaultData: Array<any> = [];

export default function TankList({ context }: { context: AppContext }) {
  ...
  const filter = useMemo(() => {
    return DataTableQueryParams.getFilter<
      z.input<typeof tankFilterInputSchema>
    >(searchParams, tankFilterInputSchema);
  }, [searchParams]);

  const columns: ColumnDef<TankWithRelationships>[] = [
    {
    ...
    },
    {
      ...
      meta: {
        title: dictionary.tank.fields.capacity,
      },
  ...
  const query = useQuery({
    queryKey: ['tank', 'list', filter, sorting, pagination],
  ...
  const table = useReactTable({
    ...
    data: query.data?.tanks || defaultData,
    columns,
  ...
  return (
    <div className="mb-4 flex w-full flex-col gap-4">
      <div className="flex items-center justify-between">
        <Breadcrumb items={[[dictionary.tank.list.menu]]} />
      ...
      <TankListFilter context={context} isLoading={query.isLoading} />
      ...
      <DataTable
        ...
        notFoundText={dictionary.tank.list.noResults}
        newButton={<TankNewButton context={context} />}
      />
