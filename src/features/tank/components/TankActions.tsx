...
import { tankDestroyManyApiCall } from 'src/features/tank/tankApiCalls';
...
export function TankActions({
  ...
  const hasPermissionToEdit = hasPermission(permissions.tankUpdate, context);
  ...
  const destroyMutation = useMutation({
    mutationFn: () => {
      return tankDestroyManyApiCall([tank.id]);
    },
    onSuccess: () => {
      ...
      toast({
        description: dictionary.tank.destroy.success,
      });
    },
    onError: (error: any) => {
      toast({
  ...
  return (
    <div className="flex justify-end gap-2">
        ...
        <DropdownMenuContent align="end" className="w-[160px]">
          {mode === 'table' && (
            <DropdownMenuItem asChild>
              <Link href={`/tank/${tank.id}`} prefetch={false}>
          ...
          {mode === 'table' && hasPermissionToEdit && (
            <DropdownMenuItem asChild>
              <Link href={`/tank/${tank.id}/edit`} prefetch={false}>
      ...
      {destroyDialogOpen && (
        <ConfirmDialog
          title={dictionary.tank.destroy.confirmTitle}
