...
import { tankFilterFormSchema } from 'src/features/tank/tankSchemas';
...
function TankListFilter({
  ...
  const previewRenders = {
    name: {
      label: dictionary.tank.fields.name,
    },
    capacityRange: {
      label: dictionary.tank.fields.capacity,
  ...
  const filter = useMemo(
    () =>
      DataTableQueryParams.getFilter<z.infer<typeof tankFilterFormSchema>>(
        searchParams,
        tankFilterFormSchema,
      ),
    [searchParams],
  );

  const form = useForm({
    resolver: zodResolver(tankFilterFormSchema),
    mode: 'onSubmit',
          ...
          <form onSubmit={form.handleSubmit(onSubmit)}>
            <div className="grid grid-cols-1 gap-4 md:grid-cols-2">
              <FormField
                ...
                render={({ field }) => (
                  <FormItem>
                    <FormLabel>{dictionary.tank.fields.name}</FormLabel>
              ...
              <FormField
                ...
                render={({ field }) => (
                  <FormItem>
                    <FormLabel>{dictionary.tank.fields.capacity}</FormLabel>
                    <RangeInput
                      type="number"
                      dictionary={dictionary}
                      disabled={isLoading}
