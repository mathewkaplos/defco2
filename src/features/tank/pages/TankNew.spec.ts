...
async function goToTankNew(page: Page) {
  ...
  await page.goto(`/tank/new`);
  await page.waitForURL(`/tank/new`);
}

test.beforeEach(async ({ page }) => {
  await testCleanupDatabase();
  await setupAdminForTest();
  await goToTankNew(page);
});

test('renders page', async ({ page }) => {
  await expect(
    page.getByRole('heading', { name: dictionary.tank.new.title }),
  ).toBeTruthy();
});
