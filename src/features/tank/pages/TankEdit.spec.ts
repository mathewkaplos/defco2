...
async function goToTankForm(page: Page) {
  ...
  // TODO: Create tank here
  // await prisma.tank.create({});

  await page.waitForURL('/');
  await page.goto(`/tank/${tank.id}/edit`);
  await page.waitForURL(`/tank/${tank.id}/edit`);
}

test.beforeEach(async ({ page }) => {
  await testCleanupDatabase();
  await setupAdminForTest();
  await goToTankForm(page);
});
