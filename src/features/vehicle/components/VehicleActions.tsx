...
import { vehicleDestroyManyApiCall } from 'src/features/vehicle/vehicleApiCalls';
...
export function VehicleActions({
  ...
  const hasPermissionToEdit = hasPermission(permissions.vehicleUpdate, context);
  ...
  const destroyMutation = useMutation({
    mutationFn: () => {
      return vehicleDestroyManyApiCall([vehicle.id]);
    },
    onSuccess: () => {
      ...
      toast({
        description: dictionary.vehicle.destroy.success,
      });
    },
    onError: (error: any) => {
      toast({
  ...
  return (
    <div className="flex justify-end gap-2">
        ...
        <DropdownMenuContent align="end" className="w-[160px]">
          {mode === 'table' && (
            <DropdownMenuItem asChild>
              <Link href={`/vehicle/${vehicle.id}`} prefetch={false}>
          ...
          {mode === 'table' && hasPermissionToEdit && (
            <DropdownMenuItem asChild>
              <Link href={`/vehicle/${vehicle.id}/edit`} prefetch={false}>
      ...
      {destroyDialogOpen && (
        <ConfirmDialog
          title={dictionary.vehicle.destroy.confirmTitle}
