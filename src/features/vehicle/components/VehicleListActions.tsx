...
import {
  vehicleDestroyManyApiCall,
  vehicleFindManyApiCall,
} from 'src/features/vehicle/vehicleApiCalls';
import { vehicleExporterMapper } from 'src/features/vehicle/vehicleExporterMapper';
import { vehicleFilterInputSchema } from 'src/features/vehicle/vehicleSchemas';
...
export default function VehicleListActions({
  ...
  filter: z.input<typeof vehicleFilterInputSchema>;
  ...
  const hasPermissionToCreate = hasPermission(permissions.vehicleCreate, context);
  ...
  const hasPermissionToImport = hasPermission(permissions.vehicleImport, context);
  ...
  const exportMutation = useMutation({
    ...
    onSuccess: (data) => {
      csvExporter(
        vehicleExporterMapper(data.vehicles, context),
        dictionary.vehicle.fields,
        'vehicles',
      );
      toast({
        description: dictionary.vehicle.export.success,
      });
    },
    onError: (error: any) => {
      toast({
  ...
  const destroyMutation = useMutation({
    mutationFn: () => {
      ...
      if (!ids.length) {
        throw new Error(context.dictionary.vehicle.destroyMany.noSelection);
      }

      return vehicleDestroyManyApiCall(ids);
    },
    onSuccess: () => {
      ...
      toast({
        description: dictionary.vehicle.destroyMany.success,
      });
    },
    onError: (error: any) => {
      toast({
  ...
  return (
    <>
        ...
        <DropdownMenuContent>
          ...
          {hasPermissionToImport && (
            <DropdownMenuItem asChild>
              <Link href={`/vehicle/importer`} prefetch={false}>
      ...
      {destroyManyDialogOpen && (
        <ConfirmDialog
          title={dictionary.vehicle.destroyMany.confirmTitle}
          description={formatTranslation(
            dictionary.vehicle.destroyMany.confirmDescription,
            selectedCount,
          )}
