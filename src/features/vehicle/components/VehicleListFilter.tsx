...
import { vehicleFilterFormSchema } from 'src/features/vehicle/vehicleSchemas';
...
function VehicleListFilter({
  ...
  const previewRenders = {
    make: {
      label: dictionary.vehicle.fields.make,
    },
    regNo: {
      label: dictionary.vehicle.fields.regNo,
    },
    ccRange: {
      label: dictionary.vehicle.fields.cc,
      render: dataTableFilterRenders(context).range(),
    },
    fullTankRange: {
      label: dictionary.vehicle.fields.fullTank,
      render: dataTableFilterRenders(context).range(),
    },
    approved: {
      label: dictionary.vehicle.fields.approved,
  ...
  const filter = useMemo(
    () =>
      DataTableQueryParams.getFilter<z.infer<typeof vehicleFilterFormSchema>>(
        searchParams,
        vehicleFilterFormSchema,
      ),
    [searchParams],
  );

  const form = useForm({
    resolver: zodResolver(vehicleFilterFormSchema),
    mode: 'onSubmit',
  ...
  useEffect(() => {
    form.reset({ ...emptyValues, ...filter } as z.infer<
      typeof vehicleFilterFormSchema
    >);
  }, [filter, form]);
          ...
          <form onSubmit={form.handleSubmit(onSubmit)}>
            <div className="grid grid-cols-1 gap-4 md:grid-cols-2">
              <FormField
                ...
                render={({ field }) => (
                  <FormItem>
                    <FormLabel>{dictionary.vehicle.fields.make}</FormLabel>
              ...
              <FormField
                ...
                render={({ field }) => (
                  <FormItem>
                    <FormLabel>{dictionary.vehicle.fields.regNo}</FormLabel>
                    <Input disabled={isLoading} {...field} />
                    <FormMessage />
                  </FormItem>
                )}
              ...
              <FormField
                ...
                render={({ field }) => (
                  <FormItem>
                    <FormLabel>{dictionary.vehicle.fields.cc}</FormLabel>
                    <RangeInput
                      type="number"
                      dictionary={dictionary}
                      disabled={isLoading}
              ...
              <FormField
                ...
                render={({ field }) => (
                  <FormItem>
                    <FormLabel>{dictionary.vehicle.fields.fullTank}</FormLabel>
                    <RangeInput
                      type="number"
                      dictionary={dictionary}
                      disabled={isLoading}
