...
import VehicleListActions from 'src/features/vehicle/components/VehicleListActions';
import VehicleListFilter from 'src/features/vehicle/components/VehicleListFilter';
import { vehicleFindManyApiCall } from 'src/features/vehicle/vehicleApiCalls';
import {
  VehicleWithRelationships,
  vehicleFilterInputSchema,
...
import { VehicleNewButton } from 'src/features/vehicle/components/VehicleNewButton';
import { z } from 'zod';
import { Vehicle } from '@prisma/client';

const defaultData: Array<any> = [];

export default function VehicleList({ context }: { context: AppContext }) {
  ...
  const filter = useMemo(() => {
    return DataTableQueryParams.getFilter<
      z.input<typeof vehicleFilterInputSchema>
    >(searchParams, vehicleFilterInputSchema);
  }, [searchParams]);

  const columns: ColumnDef<VehicleWithRelationships>[] = [
    {
    ...
    },
    {
      accessorKey: 'make',
      meta: {
        title: dictionary.vehicle.fields.make,
      },
    },
    {
      accessorKey: 'regNo',
      meta: {
        title: dictionary.vehicle.fields.regNo,
      },
      cell: ({ getValue, row }) => (
    ...
    },
    {
      accessorKey: 'cc',
      meta: {
        title: dictionary.vehicle.fields.cc,
      },
    ...
    },
    {
      ...
      meta: {
        title: dictionary.vehicle.fields.fullTank,
      },
      header: dataTableHeader('right', dictionary),
      cell: ({ getValue }) => {
        return (
    ...
    },
    {
      ...
      meta: {
        title: dictionary.vehicle.fields.approved,
      },
      cell: ({ row }) => {
        return row.getValue('approved')
  ...
  const query = useQuery({
    queryKey: ['vehicle', 'list', filter, sorting, pagination],
    queryFn: async ({ signal }) => {
      return vehicleFindManyApiCall(
        {
          filter: filter,
  ...
  const table = useReactTable({
    ...
    data: query.data?.vehicles || defaultData,
    columns,
  ...
  return (
    <div className="mb-4 flex w-full flex-col gap-4">
      <div className="flex items-center justify-between">
        <Breadcrumb items={[[dictionary.vehicle.list.menu]]} />
      ...
      <VehicleListFilter context={context} isLoading={query.isLoading} />
      ...
      <DataTable
        ...
        notFoundText={dictionary.vehicle.list.noResults}
        newButton={<VehicleNewButton context={context} />}
      />
