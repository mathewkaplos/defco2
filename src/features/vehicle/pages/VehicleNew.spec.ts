...
async function goToVehicleNew(page: Page) {
  ...
  await page.waitForURL(`/vehicle/new`);
}

test.beforeEach(async ({ page }) => {
  await testCleanupDatabase();
  await setupAdminForTest();
  await goToVehicleNew(page);
});

test('renders page', async ({ page }) => {
  await expect(
    page.getByRole('heading', { name: dictionary.vehicle.new.title }),
  ).toBeTruthy();
});
