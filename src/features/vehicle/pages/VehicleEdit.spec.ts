...
async function goToVehicleForm(page: Page) {
  ...
  await page.goto(`/vehicle/${vehicle.id}/edit`);
  await page.waitForURL(`/vehicle/${vehicle.id}/edit`);
}

test.beforeEach(async ({ page }) => {
  await testCleanupDatabase();
  await setupAdminForTest();
  await goToVehicleForm(page);
});
