...
async function goToDeviceNew(page: Page) {
  ...
  await page.goto(`/device/new`);
  await page.waitForURL(`/device/new`);
}

test.beforeEach(async ({ page }) => {
  await testCleanupDatabase();
  await setupAdminForTest();
  await goToDeviceNew(page);
});

test('renders page', async ({ page }) => {
  await expect(
    page.getByRole('heading', { name: dictionary.device.new.title }),
  ).toBeTruthy();
});
