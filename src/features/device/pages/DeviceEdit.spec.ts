...
async function goToDeviceForm(page: Page) {
  ...
  // TODO: Create device here
  // await prisma.device.create({});

  await page.waitForURL('/');
  await page.goto(`/device/${device.id}/edit`);
  await page.waitForURL(`/device/${device.id}/edit`);
}

test.beforeEach(async ({ page }) => {
  await testCleanupDatabase();
  await setupAdminForTest();
  await goToDeviceForm(page);
});
