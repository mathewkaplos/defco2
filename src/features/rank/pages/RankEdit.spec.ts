...
async function goToRankForm(page: Page) {
  ...
  // TODO: Create rank here
  // await prisma.rank.create({});

  await page.waitForURL('/');
  await page.goto(`/rank/${rank.id}/edit`);
  await page.waitForURL(`/rank/${rank.id}/edit`);
}

test.beforeEach(async ({ page }) => {
  await testCleanupDatabase();
  await setupAdminForTest();
  await goToRankForm(page);
});
