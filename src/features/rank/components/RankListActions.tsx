...
import {
  rankDestroyManyApiCall,
  rankFindManyApiCall,
} from 'src/features/rank/rankApiCalls';
import { rankExporterMapper } from 'src/features/rank/rankExporterMapper';
import { rankFilterInputSchema } from 'src/features/rank/rankSchemas';
...
export default function RankListActions({
  context,
  table,
  count,
  filter,
  ...
  filter: z.input<typeof rankFilterInputSchema>;
  ...
  const hasPermissionToCreate = hasPermission(permissions.rankCreate, context);
  ...
  const hasPermissionToImport = hasPermission(permissions.rankImport, context);
  ...
  const exportMutation = useMutation({
    ...
    onSuccess: (data) => {
      csvExporter(
        rankExporterMapper(data.ranks, context),
        dictionary.rank.fields,
        'ranks',
      );
      toast({
        description: dictionary.rank.export.success,
      });
    },
    onError: (error: any) => {
      toast({
  ...
  const destroyMutation = useMutation({
    mutationFn: () => {
      ...
      if (!ids.length) {
        throw new Error(context.dictionary.rank.destroyMany.noSelection);
      }

      return rankDestroyManyApiCall(ids);
    },
    onSuccess: () => {
      ...
      toast({
        description: dictionary.rank.destroyMany.success,
      });
    },
    onError: (error: any) => {
      toast({
  ...
  return (
    <>
        ...
        <DropdownMenuContent>
          ...
          {hasPermissionToImport && (
            <DropdownMenuItem asChild>
              <Link href={`/rank/importer`} prefetch={false}>
      ...
      {destroyManyDialogOpen && (
        <ConfirmDialog
          title={dictionary.rank.destroyMany.confirmTitle}
          description={formatTranslation(
            dictionary.rank.destroyMany.confirmDescription,
            selectedCount,
          )}
