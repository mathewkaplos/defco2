...
async function goToCustomerForm(page: Page) {
  ...
  await page.goto(`/customer/${customer.id}/edit`);
  await page.waitForURL(`/customer/${customer.id}/edit`);
}

test.beforeEach(async ({ page }) => {
  await testCleanupDatabase();
  await setupAdminForTest();
  await goToCustomerForm(page);
});
