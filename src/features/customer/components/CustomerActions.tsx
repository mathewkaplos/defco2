...
import { customerDestroyManyApiCall } from 'src/features/customer/customerApiCalls';
...
export function CustomerActions({
  ...
  const hasPermissionToEdit = hasPermission(permissions.customerUpdate, context);
  ...
  const destroyMutation = useMutation({
    mutationFn: () => {
      return customerDestroyManyApiCall([customer.id]);
    },
    onSuccess: () => {
      ...
      toast({
        description: dictionary.customer.destroy.success,
      });
    },
    onError: (error: any) => {
      toast({
  ...
  return (
    <div className="flex justify-end gap-2">
        ...
        <DropdownMenuContent align="end" className="w-[160px]">
          {mode === 'table' && (
            <DropdownMenuItem asChild>
              <Link href={`/customer/${customer.id}`} prefetch={false}>
          ...
          {mode === 'table' && hasPermissionToEdit && (
            <DropdownMenuItem asChild>
              <Link href={`/customer/${customer.id}/edit`} prefetch={false}>
      ...
      {destroyDialogOpen && (
        <ConfirmDialog
          title={dictionary.customer.destroy.confirmTitle}
