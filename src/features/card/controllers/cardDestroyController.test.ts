import { Card, Tenant, User } from '@prisma/client';
...
async function createCard(context: AppContext): Promise<any> {
...
describe('cardDestroy', () => {
  ...
  it('must be signed in', async () => {
    try {
      await cardDestroyManyController(
        {
          ids: [card?.id],
        },
        await testContext(),
  ...
  it.skip('deletes', async () => {
    const countBeforeDelete = await prisma.card.count();
    ...
    const countAfterDelete = await prisma.card.count();
