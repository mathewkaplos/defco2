...
import { cardDestroyManyApiCall } from 'src/features/card/cardApiCalls';
...
export function CardActions({
  ...
  const hasPermissionToEdit = hasPermission(permissions.cardUpdate, context);
  ...
  const destroyMutation = useMutation({
    mutationFn: () => {
      return cardDestroyManyApiCall([card.id]);
    },
    onSuccess: () => {
      ...
      toast({
        description: dictionary.card.destroy.success,
      });
    },
    onError: (error: any) => {
      toast({
  ...
  return (
    <div className="flex justify-end gap-2">
        ...
        <DropdownMenuContent align="end" className="w-[160px]">
          {mode === 'table' && (
            <DropdownMenuItem asChild>
              <Link href={`/card/${card.id}`} prefetch={false}>
          ...
          {mode === 'table' && hasPermissionToEdit && (
            <DropdownMenuItem asChild>
              <Link href={`/card/${card.id}/edit`} prefetch={false}>
      ...
      {destroyDialogOpen && (
        <ConfirmDialog
          title={dictionary.card.destroy.confirmTitle}
