...
import { dispenserDestroyManyApiCall } from 'src/features/dispenser/dispenserApiCalls';
...
export function DispenserActions({
  mode,
  dispenser,
  context,
}: {
  ...
  const hasPermissionToEdit = hasPermission(permissions.dispenserUpdate, context);
  ...
  const destroyMutation = useMutation({
    mutationFn: () => {
      return dispenserDestroyManyApiCall([dispenser.id]);
    },
    onSuccess: () => {
      ...
      toast({
        description: dictionary.dispenser.destroy.success,
      });
    },
    onError: (error: any) => {
      toast({
  ...
  return (
    <div className="flex justify-end gap-2">
        ...
        <DropdownMenuContent align="end" className="w-[160px]">
          {mode === 'table' && (
            <DropdownMenuItem asChild>
              <Link href={`/dispenser/${dispenser.id}`} prefetch={false}>
          ...
          {mode === 'table' && hasPermissionToEdit && (
            <DropdownMenuItem asChild>
              <Link href={`/dispenser/${dispenser.id}/edit`} prefetch={false}>
      ...
      {destroyDialogOpen && (
        <ConfirmDialog
          title={dictionary.dispenser.destroy.confirmTitle}
