...
import {
  dispenserDestroyManyApiCall,
  dispenserFindManyApiCall,
} from 'src/features/dispenser/dispenserApiCalls';
import { dispenserExporterMapper } from 'src/features/dispenser/dispenserExporterMapper';
import { dispenserFilterInputSchema } from 'src/features/dispenser/dispenserSchemas';
...
export default function DispenserListActions({
  context,
  table,
  count,
  filter,
  ...
  filter: z.input<typeof dispenserFilterInputSchema>;
  sorting: DataTableSort;
  count?: number;
  context: AppContext;
  table: Table<Dispenser>;
}) {
  ...
  const hasPermissionToCreate = hasPermission(permissions.dispenserCreate, context);
  ...
  const hasPermissionToImport = hasPermission(permissions.dispenserImport, context);
  ...
  const exportMutation = useMutation({
    ...
    onSuccess: (data) => {
      csvExporter(
        dispenserExporterMapper(data.dispensers, context),
        dictionary.dispenser.fields,
        'dispensers',
      );
      toast({
        description: dictionary.dispenser.export.success,
      });
    },
    onError: (error: any) => {
      toast({
  ...
  const destroyMutation = useMutation({
    mutationFn: () => {
      ...
      if (!ids.length) {
        throw new Error(context.dictionary.dispenser.destroyMany.noSelection);
      }

      return dispenserDestroyManyApiCall(ids);
    },
    onSuccess: () => {
      ...
      toast({
        description: dictionary.dispenser.destroyMany.success,
      });
    },
    onError: (error: any) => {
      toast({
  ...
  return (
    <>
        ...
        <DropdownMenuContent>
          ...
          {hasPermissionToImport && (
            <DropdownMenuItem asChild>
              <Link href={`/dispenser/importer`} prefetch={false}>
      ...
      {destroyManyDialogOpen && (
        <ConfirmDialog
          title={dictionary.dispenser.destroyMany.confirmTitle}
          description={formatTranslation(
            dictionary.dispenser.destroyMany.confirmDescription,
            selectedCount,
          )}
