...
async function goToDispenserForm(page: Page) {
  ...
  await page.goto(`/dispenser/${dispenser.id}/edit`);
  await page.waitForURL(`/dispenser/${dispenser.id}/edit`);
}

test.beforeEach(async ({ page }) => {
  await testCleanupDatabase();
  await setupAdminForTest();
  await goToDispenserForm(page);
});
