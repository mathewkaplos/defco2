...
async function goToDispenserNew(page: Page) {
  ...
  await page.goto(`/dispenser/new`);
  await page.waitForURL(`/dispenser/new`);
}

test.beforeEach(async ({ page }) => {
  await testCleanupDatabase();
  await setupAdminForTest();
  await goToDispenserNew(page);
});

test('renders page', async ({ page }) => {
  await expect(
    page.getByRole('heading', { name: dictionary.dispenser.new.title }),
  ).toBeTruthy();
});
