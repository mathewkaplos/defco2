...
async function goToDispenserView(page: Page) {
  ...
  await page.goto(`/dispenser/${dispenser.id}`);
  await page.waitForURL(`/dispenser/${dispenser.id}`);
}

test.beforeEach(async ({ page }) => {
  await testCleanupDatabase();
  await setupAdminForTest();
  await goToDispenserView(page);
});
