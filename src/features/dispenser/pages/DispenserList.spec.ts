...
async function goToDispenserList(page: Page) {
  ...
  await page.goto(`/dispenser`);
  await page.waitForURL(`/dispenser`);
}

test.beforeEach(async ({ page }) => {
  await testCleanupDatabase();
  await setupAdminForTest();
  await goToDispenserList(page);
});

test('renders page', async ({ page }) => {
  await expect(
    page.getByRole('heading', { name: dictionary.dispenser.list.title }),
  ).toBeTruthy();
});
