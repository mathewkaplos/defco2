...
async function createProductList(context: AppContext) {
...
describe('productFindMany', () => {
  ...
  beforeEach(async () => {
    ...
    const output = await createProductList(
      await testContext({
  ...
  it('must be signed in', async () => {
    try {
      await productFindManyController({}, await testContext());
      fail();
    } catch (error: any) {
  ...
  it('must have permission', async () => {
    ...
    try {
      await productFindManyController(
        {},
        await testContext({
          currentUserId: currentUser?.id,
        }),
