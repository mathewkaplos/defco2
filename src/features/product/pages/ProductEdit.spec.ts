...
async function goToProductForm(page: Page) {
  ...
  await page.goto(`/product/${product.id}/edit`);
  await page.waitForURL(`/product/${product.id}/edit`);
}

test.beforeEach(async ({ page }) => {
  await testCleanupDatabase();
  await setupAdminForTest();
  await goToProductForm(page);
});
