...
import { productDestroyManyApiCall } from 'src/features/product/productApiCalls';
...
export function ProductActions({
  ...
  const hasPermissionToEdit = hasPermission(permissions.productUpdate, context);
  ...
  const destroyMutation = useMutation({
    mutationFn: () => {
      return productDestroyManyApiCall([product.id]);
    },
    onSuccess: () => {
      ...
      toast({
        description: dictionary.product.destroy.success,
      });
    },
    onError: (error: any) => {
      toast({
  ...
  return (
    <div className="flex justify-end gap-2">
      <DropdownMenu>
        ...
        <DropdownMenuContent align="end" className="w-[160px]">
          ...
          {mode === 'table' && hasPermissionToEdit && (
            <DropdownMenuItem asChild>
              <Link href={`/product/${product.id}/edit`} prefetch={false}>
      ...
      {destroyDialogOpen && (
        <ConfirmDialog
          title={dictionary.product.destroy.confirmTitle}
