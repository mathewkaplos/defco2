...
import ProductListActions from 'src/features/product/components/ProductListActions';
import ProductListFilter from 'src/features/product/components/ProductListFilter';
...
import { ProductNewButton } from 'src/features/product/components/ProductNewButton';
import { z } from 'zod';
...
export default function ProductList({ context }: { context: AppContext }) {
  ...
  const filter = useMemo(() => {
    return DataTableQueryParams.getFilter<
      z.input<typeof productFilterInputSchema>
    >(searchParams, productFilterInputSchema);
  }, [searchParams]);

  const columns: ColumnDef<ProductWithRelationships>[] = [
    {
    ...
    },
    {
      accessorKey: 'name',
      meta: {
        title: dictionary.product.fields.name,
      },
      cell: ({ getValue, row }) => (
    ...
    },
    {
      accessorKey: 'price',
      meta: {
        title: dictionary.product.fields.price,
      },
  ...
  const query = useQuery({
    queryKey: ['product', 'list', filter, sorting, pagination],
  ...
  const table = useReactTable({
    ...
    data: query.data?.products || defaultData,
    columns,
  ...
  return (
    <div className="mb-4 flex w-full flex-col gap-4">
      <div className="flex items-center justify-between">
        <Breadcrumb items={[[dictionary.product.list.menu]]} />
      ...
      <ProductListFilter context={context} isLoading={query.isLoading} />
      ...
      <DataTable
        ...
        notFoundText={dictionary.product.list.noResults}
        newButton={<ProductNewButton context={context} />}
      />
