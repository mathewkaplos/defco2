...
import {
  productDestroyManyApiCall,
  productFindManyApiCall,
} from 'src/features/product/productApiCalls';
import { productExporterMapper } from 'src/features/product/productExporterMapper';
import { productFilterInputSchema } from 'src/features/product/productSchemas';
...
export default function ProductListActions({
  ...
  filter: z.input<typeof productFilterInputSchema>;
  ...
  const hasPermissionToCreate = hasPermission(permissions.productCreate, context);
  ...
  const hasPermissionToImport = hasPermission(permissions.productImport, context);

  const exportMutation = useMutation({
    mutationFn: () => {
      return productFindManyApiCall({
        filter: filter,
    ...
    onSuccess: (data) => {
      csvExporter(
        productExporterMapper(data.products, context),
        dictionary.product.fields,
        'products',
      );
      toast({
        description: dictionary.product.export.success,
      });
    },
    onError: (error: any) => {
      toast({
  ...
  const destroyMutation = useMutation({
    mutationFn: () => {
      ...
      if (!ids.length) {
        throw new Error(context.dictionary.product.destroyMany.noSelection);
      }

      return productDestroyManyApiCall(ids);
    },
    onSuccess: () => {
      ...
      toast({
        description: dictionary.product.destroyMany.success,
      });
    },
    onError: (error: any) => {
      toast({
  ...
  return (
    <>
        ...
        <DropdownMenuContent>
          ...
          {hasPermissionToImport && (
            <DropdownMenuItem asChild>
              <Link href={`/product/importer`} prefetch={false}>
      ...
      {destroyManyDialogOpen && (
        <ConfirmDialog
          title={dictionary.product.destroyMany.confirmTitle}
          description={formatTranslation(
            dictionary.product.destroyMany.confirmDescription,
            selectedCount,
          )}
