...
import { stationDestroyManyApiCall } from 'src/features/station/stationApiCalls';
...
export function StationActions({
  ...
  const hasPermissionToEdit = hasPermission(permissions.stationUpdate, context);
  ...
  const destroyMutation = useMutation({
    mutationFn: () => {
      return stationDestroyManyApiCall([station.id]);
    },
    onSuccess: () => {
      ...
      toast({
        description: dictionary.station.destroy.success,
      });
    },
    onError: (error: any) => {
      toast({
  ...
  return (
    <div className="flex justify-end gap-2">
        ...
        <DropdownMenuContent align="end" className="w-[160px]">
          {mode === 'table' && (
            <DropdownMenuItem asChild>
              <Link href={`/station/${station.id}`} prefetch={false}>
          ...
          {mode === 'table' && hasPermissionToEdit && (
            <DropdownMenuItem asChild>
              <Link href={`/station/${station.id}/edit`} prefetch={false}>
      ...
      {destroyDialogOpen && (
        <ConfirmDialog
          title={dictionary.station.destroy.confirmTitle}
