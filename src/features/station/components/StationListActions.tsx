...
import {
  stationDestroyManyApiCall,
  stationFindManyApiCall,
} from 'src/features/station/stationApiCalls';
import { stationExporterMapper } from 'src/features/station/stationExporterMapper';
import { stationFilterInputSchema } from 'src/features/station/stationSchemas';
...
export default function StationListActions({
  context,
  table,
  count,
  filter,
  ...
  filter: z.input<typeof stationFilterInputSchema>;
  ...
  const hasPermissionToCreate = hasPermission(permissions.stationCreate, context);
  ...
  const hasPermissionToImport = hasPermission(permissions.stationImport, context);
  ...
  const exportMutation = useMutation({
    ...
    onSuccess: (data) => {
      csvExporter(
        stationExporterMapper(data.stations, context),
        dictionary.station.fields,
        'stations',
      );
      toast({
        description: dictionary.station.export.success,
      });
    },
    onError: (error: any) => {
      toast({
  ...
  const destroyMutation = useMutation({
    mutationFn: () => {
      ...
      if (!ids.length) {
        throw new Error(context.dictionary.station.destroyMany.noSelection);
      }

      return stationDestroyManyApiCall(ids);
    },
    onSuccess: () => {
      ...
      toast({
        description: dictionary.station.destroyMany.success,
      });
    },
    onError: (error: any) => {
      toast({
  ...
  return (
    <>
        ...
        <DropdownMenuContent>
          ...
          {hasPermissionToImport && (
            <DropdownMenuItem asChild>
              <Link href={`/station/importer`} prefetch={false}>
      ...
      {destroyManyDialogOpen && (
        <ConfirmDialog
          title={dictionary.station.destroyMany.confirmTitle}
          description={formatTranslation(
            dictionary.station.destroyMany.confirmDescription,
            selectedCount,
          )}
