...
async function goToStationForm(page: Page) {
  ...
  // TODO: Create station here
  // await prisma.station.create({});

  await page.waitForURL('/');
  await page.goto(`/station/${station.id}/edit`);
  await page.waitForURL(`/station/${station.id}/edit`);
}

test.beforeEach(async ({ page }) => {
  await testCleanupDatabase();
  await setupAdminForTest();
  await goToStationForm(page);
});
