...
async function goToMembershipForm(page: Page) {
  ...
  await page.goto(`/membership/${membership.id}/edit`);
  await page.waitForURL(`/membership/${membership.id}/edit`);
}
...
test('updates membership', async ({ page }) => {
...
test(`doesn't allow admin to remove their own admin role`, async ({ page }) => {
  await page.getByTestId(`roles-value-admin-remove`).click();
  ...
  expect(
    page
      .getByText(dictionary.membership.errors.cannotRemoveSelfAdminRole)
      .first(),
  ).toBeVisible();
});
