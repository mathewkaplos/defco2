...
import { saleDestroyManyApiCall } from 'src/features/sale/saleApiCalls';
...
export function SaleActions({
  ...
  const hasPermissionToEdit = hasPermission(permissions.saleUpdate, context);
  ...
  const destroyMutation = useMutation({
    mutationFn: () => {
      return saleDestroyManyApiCall([sale.id]);
    },
    onSuccess: () => {
      ...
      toast({
        description: dictionary.sale.destroy.success,
      });
    },
    onError: (error: any) => {
      toast({
  ...
  return (
    <div className="flex justify-end gap-2">
        ...
        <DropdownMenuContent align="end" className="w-[160px]">
          {mode === 'table' && (
            <DropdownMenuItem asChild>
              <Link href={`/sale/${sale.id}`} prefetch={false}>
          ...
          {mode === 'table' && hasPermissionToEdit && (
            <DropdownMenuItem asChild>
              <Link href={`/sale/${sale.id}/edit`} prefetch={false}>
      ...
      {destroyDialogOpen && (
        <ConfirmDialog
          title={dictionary.sale.destroy.confirmTitle}
