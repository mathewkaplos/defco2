...
import {
  saleDestroyManyApiCall,
  saleFindManyApiCall,
} from 'src/features/sale/saleApiCalls';
import { saleExporterMapper } from 'src/features/sale/saleExporterMapper';
import { saleFilterInputSchema } from 'src/features/sale/saleSchemas';
...
export default function SaleListActions({
  context,
  table,
  count,
  filter,
  ...
  filter: z.input<typeof saleFilterInputSchema>;
  ...
  const hasPermissionToCreate = hasPermission(permissions.saleCreate, context);
  ...
  const hasPermissionToImport = hasPermission(permissions.saleImport, context);
  ...
  const exportMutation = useMutation({
    ...
    onSuccess: (data) => {
      csvExporter(
        saleExporterMapper(data.sales, context),
        dictionary.sale.fields,
        'sales',
      );
      toast({
        description: dictionary.sale.export.success,
      });
    },
    onError: (error: any) => {
      toast({
  ...
  const destroyMutation = useMutation({
    mutationFn: () => {
      ...
      if (!ids.length) {
        throw new Error(context.dictionary.sale.destroyMany.noSelection);
      }

      return saleDestroyManyApiCall(ids);
    },
    onSuccess: () => {
      ...
      toast({
        description: dictionary.sale.destroyMany.success,
      });
    },
    onError: (error: any) => {
      toast({
  ...
  return (
    <>
        ...
        <DropdownMenuContent>
          ...
          {hasPermissionToImport && (
            <DropdownMenuItem asChild>
              <Link href={`/sale/importer`} prefetch={false}>
      ...
      {destroyManyDialogOpen && (
        <ConfirmDialog
          title={dictionary.sale.destroyMany.confirmTitle}
          description={formatTranslation(
            dictionary.sale.destroyMany.confirmDescription,
            selectedCount,
          )}
