...
async function goToMaterialReceiptForm(page: Page) {
  ...
  await page.goto(`/material-receipt/${materialReceipt.id}/edit`);
  await page.waitForURL(`/material-receipt/${materialReceipt.id}/edit`);
}

test.beforeEach(async ({ page }) => {
  await testCleanupDatabase();
  await setupAdminForTest();
  await goToMaterialReceiptForm(page);
});
