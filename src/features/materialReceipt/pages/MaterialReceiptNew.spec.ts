...
async function goToMaterialReceiptNew(page: Page) {
  ...
  await page.goto(`/material-receipt/new`);
  await page.waitForURL(`/material-receipt/new`);
}

test.beforeEach(async ({ page }) => {
  await testCleanupDatabase();
  await setupAdminForTest();
  await goToMaterialReceiptNew(page);
});

test('renders page', async ({ page }) => {
  await expect(
    page.getByRole('heading', { name: dictionary.materialReceipt.new.title }),
  ).toBeTruthy();
});
