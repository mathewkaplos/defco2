...
async function goToMaterialReceiptList(page: Page) {
  ...
  await page.waitForURL(`/material-receipt`);
}

test.beforeEach(async ({ page }) => {
  await testCleanupDatabase();
  await setupAdminForTest();
  await goToMaterialReceiptList(page);
});

test('renders page', async ({ page }) => {
  await expect(
    page.getByRole('heading', { name: dictionary.materialReceipt.list.title }),
  ).toBeTruthy();
});
