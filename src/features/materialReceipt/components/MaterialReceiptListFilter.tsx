...
import { materialReceiptFilterFormSchema } from 'src/features/materialReceipt/materialReceiptSchemas';
...
function MaterialReceiptListFilter({
  context,
  isLoading,
}: {
  context: AppContext;
  ...
  const previewRenders = {
    date1Range: {
      label: dictionary.materialReceipt.fields.date1,
      render: dataTableFilterRenders(context).dateRange(),
    },
    supplier: {
      label: dictionary.materialReceipt.fields.supplier,
    },
    quantityRange: {
      label: dictionary.materialReceipt.fields.quantity,
      render: dataTableFilterRenders(context).range(),
    },
    priceRange: {
      label: dictionary.materialReceipt.fields.price,
      render: dataTableFilterRenders(context).decimalRange(),
    },
    totalRange: {
      label: dictionary.materialReceipt.fields.total,
      render: dataTableFilterRenders(context).decimalRange(),
    },
  };
  ...
  const filter = useMemo(
    () =>
      DataTableQueryParams.getFilter<z.infer<typeof materialReceiptFilterFormSchema>>(
        searchParams,
        materialReceiptFilterFormSchema,
      ),
    [searchParams],
  );

  const form = useForm({
    resolver: zodResolver(materialReceiptFilterFormSchema),
    mode: 'onSubmit',
  ...
  useEffect(() => {
    form.reset({ ...emptyValues, ...filter } as z.infer<
      typeof materialReceiptFilterFormSchema
    >);
  }, [filter, form]);
          ...
          <form onSubmit={form.handleSubmit(onSubmit)}>
            <div className="grid grid-cols-1 gap-4 md:grid-cols-2">
              ...
              <FormField
                ...
                render={({ field }) => (
                  <FormItem>
                    <FormLabel>{dictionary.materialReceipt.fields.supplier}</FormLabel>
              ...
              <FormField
                ...
                render={({ field }) => (
                  <FormItem>
                    <FormLabel>{dictionary.materialReceipt.fields.quantity}</FormLabel>
                    <RangeInput
                      type="number"
                      dictionary={dictionary}
                      disabled={isLoading}
              ...
              <FormField
                ...
                render={({ field }) => (
                  <FormItem>
                    <FormLabel>{dictionary.materialReceipt.fields.price}</FormLabel>
                    <RangeInput
                      type="text"
                      dictionary={dictionary}
                      disabled={isLoading}
              ...
              <FormField
                ...
                render={({ field }) => (
                  <FormItem>
                    <FormLabel>{dictionary.materialReceipt.fields.total}</FormLabel>
                    <RangeInput
                      type="text"
                      dictionary={dictionary}
                      disabled={isLoading}
...
export default MaterialReceiptListFilter;
